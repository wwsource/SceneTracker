...
class CorrBlock:
    def __init__(self, fmaps, num_levels=4, radius=4):
        B, S, C, H, W = fmaps.shape  # shape: (B, S, C, h, w)
    ...
    def sample(self, coords):
        ...
        for i in range(self.num_levels):
            ...
            centroid_lvl = coords.reshape(B * S * N, 1, 1, 2) / 2 ** i  # shape: (B*S*N, 1, 1, 2)
            delta_lvl = delta.view(1, 2 * r + 1, 2 * r + 1, 2)  # shape: (1, l, l, 2)
            coords_lvl = centroid_lvl + delta_lvl  # shape: (B*S*N, l, l, 2)
    ...
    def corr(self, targets):
        B, S, N, C = targets.shape  # shape: (B, S, N, C)
        assert C == self.C
        assert S == self.S
        ...
        for fmaps in self.fmaps_pyramid:
            ...
            fmap2s = fmaps.view(B, S, C, H * W)  # shape: (B, S, C, h*w)
...
class UpdateFormer(nn.Module):
    ...
    def forward(self, input_tensor):
        x = self.input_transform(input_tensor)  # shape: (B, N_before_window_end, S, hidden_dim)

        j = 0
        for i in range(len(self.time_blocks)):
            B, N, T, _ = x.shape
            x_time = rearrange(x, "b n t c -> (b n) t c", b=B, t=T, n=N)  # shape: (B*N_before_window_end, S, hidden_dim)
            x_time = self.time_blocks[i](x_time)

            x = rearrange(x_time, "(b n) t c -> b n t c ", b=B, t=T, n=N)  # shape: (B, N_before_window_end, S, hidden_dim)
            if self.add_space_attn and (
                i % (len(self.time_blocks) // len(self.space_blocks)) == 0
            ):
                x_space = rearrange(x, "b n t c -> (b t) n c ", b=B, t=T, n=N)  # shape: (B*S, N_before_window_end, hidden_dim)
                x_space = self.space_blocks[j](x_space)
                x = rearrange(x_space, "(b t) n c -> b n t c  ", b=B, t=T, n=N)  # shape: (B, N_before_window_end, S, hidden_dim)
                j += 1

        flow = self.flow_head(x)  # shape: (B, N_before_window_end, S, C+2)
        return flow
